  ...
  <div>
    ...
    >
      ...
      >
        ...
        <template #filter="{ filterModel }">
          <MultiSelect
            ...
            :options="dataState.providers.map((x) => x.name)"
            placeholder="Any"
          >
          </MultiSelect>
        </template>
      ...
      <Column
        ...
        data-type="numeric"
        sortable
        :sort-field="calculateFee"
      >
        <template #body="{ data }"> {{ data.total_fee.toFixed(2) }}% </template>
        <template #filter="{ filterModel }">
          <InputNumber
            v-model="filterModel.value"
            mode="decimal"
      ...
      >
        ...
        <template #filter="{ filterModel }">
          ...
          <div style="margin-bottom: 1rem"></div>
          <MultiSelect
            v-if="filterModel.value === null || filterModel.value[0] !== -1"
            v-model="filterModel.value"
