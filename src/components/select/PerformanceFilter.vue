...
const filterObj = computed(() => {
    ...
    if (lowerDate.value)
        obj.filter_ignore_before = Math.floor(lowerDate.value.getTime() / 1000);
    if (upperDate.value)
        obj.filter_ignore_after = Math.floor(upperDate.value.getTime() / 1000);
    return obj;
})
    ...
    <div>
        ...
        <Accordion multiple :active-index="[]">
            ...
            <AccordionTab header="Date">
                <Calendar selection-mode="single" view="month" show-button-bar v-model="lowerDate" placeholder="From"
                    class="date-range-selector" v-if="!upperDateOnly"/>
                <Calendar selection-mode="single" view="month" show-button-bar v-model="upperDate" :placeholder="upperDateOnly ? 'Latest' : 'To'"
